<?php
/**
 * Settings page at Settings → RafaelMarreca Push Dispatch.
 */
...
class FCM_Push_Settings {
	...
	const PAGE_SLUG  = 'rafaelmarreca-push-dispatch';
	...
	public static function add_menu(): void {
		add_options_page(
			__( 'RafaelMarreca Push Dispatch', 'rafaelmarreca-push-dispatch' ),
			__( 'RafaelMarreca Push Dispatch', 'rafaelmarreca-push-dispatch' ),
			'manage_options',
			self::PAGE_SLUG,
			[ __CLASS__, 'render' ]
	...
	public static function render(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to access this page.', 'rafaelmarreca-push-dispatch' ) );
		}
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'RafaelMarreca Push Dispatch', 'rafaelmarreca-push-dispatch' ); ?></h1>
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" enctype="multipart/form-data">
				...
				<h2><?php esc_html_e( 'Credentials', 'rafaelmarreca-push-dispatch' ); ?></h2>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><label for="mp_json"><?php esc_html_e( 'Service account JSON', 'rafaelmarreca-push-dispatch' ); ?></label></th>
						<td>
							<?php if ( $has_cred ) : ?>
								<p>
									...
									printf(
										...
										esc_html__( 'Configured — project: %s', 'rafaelmarreca-push-dispatch' ),
										'<code>' . esc_html( $settings['project_id'] ?: __( '(unknown)', 'rafaelmarreca-push-dispatch' ) ) . '</code>'
									);
									?>
								</p>
							<?php else : ?>
								<p style="color:#d63638">
									...
									<?php esc_html_e( 'Not configured. The plugin will not send notifications until this is set.', 'rafaelmarreca-push-dispatch' ); ?>
								</p>
							<?php endif; ?>
							...
							<p class="description">
								...
								printf(
									...
									esc_html__( 'Download from Firebase Console → ⚙ Project settings → Service accounts → Generate new private key. The file is stored outside the database in %s.', 'rafaelmarreca-push-dispatch' ),
									'<code>wp-content/uploads/rafaelmarreca-push-private/</code>'
								);
								?>
							</p>
						</td>
				...
				<h2><?php esc_html_e( 'Automatic send', 'rafaelmarreca-push-dispatch' ); ?></h2>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'On post publish', 'rafaelmarreca-push-dispatch' ); ?></th>
						<td>
							<label>
								<input type="checkbox" name="enabled_auto" value="1"
									<?php checked( ! empty( $settings['enabled_auto'] ) ); ?> />
								<?php esc_html_e( 'Send a push notification automatically when a post is published.', 'rafaelmarreca-push-dispatch' ); ?>
							</label>
							<p class="description"><?php esc_html_e( "The post's primary category determines the topic (see map below).", 'rafaelmarreca-push-dispatch' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="default_topic"><?php esc_html_e( 'Default topic', 'rafaelmarreca-push-dispatch' ); ?></label></th>
						<td>
							<input type="text" name="default_topic" id="default_topic"
								value="<?php echo esc_attr( $settings['default_topic'] ); ?>"
								class="regular-text" />
							<p class="description"><?php esc_html_e( "Used when the post's category is not in the map below.", 'rafaelmarreca-push-dispatch' ); ?></p>
						</td>
					</tr>
				</table>

				<h2><?php esc_html_e( 'Category → topic map', 'rafaelmarreca-push-dispatch' ); ?></h2>
				<p><?php esc_html_e( 'Map WordPress category IDs to the topic name your app subscribes to.', 'rafaelmarreca-push-dispatch' ); ?></p>
				<table class="form-table" role="presentation">
							...
							<th scope="row">
								<label>
									...
									printf(
										...
										esc_html__( 'Category %d', 'rafaelmarreca-push-dispatch' ),
										(int) $cat_id
									);
									?>
								</label><br />
								<small><?php
									...
									echo esc_html( $cat instanceof WP_Term ? $cat->name : __( '(not found in WordPress)', 'rafaelmarreca-push-dispatch' ) );
								?></small>
							</th>
							<td>
								<input type="text"
				...
				<?php submit_button( __( 'Save', 'rafaelmarreca-push-dispatch' ) ); ?>
			</form>

			<hr />
			<h2><?php esc_html_e( 'Test send', 'rafaelmarreca-push-dispatch' ); ?></h2>
			<p><?php esc_html_e( 'Sends a test notification with title "RafaelMarreca Push Dispatch — test" to the topic below.', 'rafaelmarreca-push-dispatch' ); ?></p>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<input type="text" name="topic" value="<?php echo esc_attr( $settings['default_topic'] ); ?>"
					class="regular-text" />
				<button type="submit" class="button" <?php disabled( ! $has_cred ); ?>><?php esc_html_e( 'Send test', 'rafaelmarreca-push-dispatch' ); ?></button>
			</form>
		</div>
		<?php
	}
	...
	public static function handle_test_send(): void {
		...
		$result = $client->send_to_topic(
			$topic,
			__( 'RafaelMarreca Push Dispatch — test', 'rafaelmarreca-push-dispatch' ),
			__( 'If you see this, your Firebase setup is working. ✅', 'rafaelmarreca-push-dispatch' ),
	...
	private static function format_notice( string $key ): string {
		$messages = [
			'saved'          => [ 'updated', __( 'Settings saved.', 'rafaelmarreca-push-dispatch' ) ],
			'upload_failed'  => [ 'error',   __( 'JSON upload failed.', 'rafaelmarreca-push-dispatch' ) ],
			'file_too_big'   => [ 'error',   __( 'File too large (max 64 KB).', 'rafaelmarreca-push-dispatch' ) ],
			'invalid_json'   => [ 'error',   __( 'File is not a valid service account JSON.', 'rafaelmarreca-push-dispatch' ) ],
			'write_failed'   => [ 'error',   __( 'Could not write file to uploads/.', 'rafaelmarreca-push-dispatch' ) ],
			'test_ok'        => [ 'updated', __( 'Test notification sent successfully.', 'rafaelmarreca-push-dispatch' ) ],
			'test_bad_topic' => [ 'error',   __( 'Invalid topic name.', 'rafaelmarreca-push-dispatch' ) ],
			'test_failed'    => [
				'error',
				sprintf(
					/* translators: %s: error message from FCM */
					__( 'Test failed: %s', 'rafaelmarreca-push-dispatch' ),
